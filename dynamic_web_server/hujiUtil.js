...
HttpRequest.prototype.setQuery = function (field, value) {
    ...
    //value.replace("+", " ");
    //var spaceIndex = value.search("\\+");
    //if (spaceIndex !== -1) {

     //   value = spaceIndex.toString();
   // }
    this.query[field] = newValue;
};
...
HttpResponse.prototype.send = function (body) {
    ...
    if (!this.sent) {
        ...
        var contentType = this.getHeader('CONTENT_TYPE');
        this.body = "";
...
HttpResponse.prototype.get = function (field) {
    this.headers[field.toUpperCase()];
};
